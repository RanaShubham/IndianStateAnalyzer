...
import com.bridgelabz.censusanalyzer.CensusAnalyzerExcepition.Exception_Error_Type;
...
public class StateCensusAnalyzer 
{
	public int loadIndianStateCensusCsv(String csvFilePath)
	{
		if(!csvFilePath.contains(".csv"))
			throw new CensusAnalyzerExcepition(Exception_Error_Type.INCORRECT_FILE_TYPE, "Only csv files are supported");
		int recordCounter = 0;
		try (Reader reader = Files.newBufferedReader(Paths.get(csvFilePath)))
		{
			...
			if(!firstLineOfFile.contains(","))
				throw new CensusAnalyzerExcepition(Exception_Error_Type.INCORRECT_DELIMETER, "Invalid delimeter");
			...
			for (int i=0; i < fileHeaderArray.length; i++)
			{
				try {
					if(!fileHeaderArray[i].equals(IndianStateCensus.headerArray[i]))
						...
						throw new CensusAnalyzerExcepition(Exception_Error_Type.INCORRECT_HEADER, "Wrong file header.");
			...
			throw new CensusAnalyzerExcepition(Exception_Error_Type.INCORRECT_PATH, "File path incorrect.");
		}
