...
import com.bridgelabz.censusanalyzer.IndianStateCensus;
import com.bridgelabz.indianstatecode.IndianStateCodeException.Exception_Error_Type;
import com.opencsv.bean.CsvToBean;
import com.opencsv.bean.CsvToBeanBuilder;

public class IndianStateCodeAnalyzer 
{
	public int loadIndianStateCodeCsv(String csvFilePath)
	{
		if(!csvFilePath.contains(".csv"))
			throw new IndianStateCodeException(Exception_Error_Type.INCORRECT_FILE_TYPE, "Only csv files are supported");
		int recordCounter = 0;
		try (Reader reader = Files.newBufferedReader(Paths.get(csvFilePath)))
		{
			...
			if(!firstLineOfFile.contains(","))
				throw new IndianStateCodeException(Exception_Error_Type.INCORRECT_DELIMETER, "Invalid delimeter");
			...
			for (int i=0; i < fileHeaderArray.length; i++)
			{
				try {
					if(!fileHeaderArray[i].equals(IndianStateCode.headerArray[i]))
						...
						throw new IndianStateCodeException(Exception_Error_Type.INCORRECT_HEADER, "Wrong file header.");
			...
			throw new IndianStateCodeException(Exception_Error_Type.INCORRECT_PATH, "File path incorrect.");
		}
